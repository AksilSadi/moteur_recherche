...
useEffect(() => {
  ...
  const fetchBooks = async () => {
    ...
    try {
      ...
      // Patch : toujours un tableau
      if (page === 1) {
        setTop3Global(Array.isArray(data.top3) ? data.top3 : []);
      }
    } finally {
      setLoading(false);
    }
...
useEffect(() => {
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      const merged: { [key: string]: Book } = {};

      recGroups.forEach((g) => {
        if (!g?.recommendations) return;
      ...
      const resultsIds = new Set(
        searchResults.map((b) => b.gutendexId)
      );

      const cleanList = Object.values(merged)
        .filter((b) => !resultsIds.has(b.gutendexId))
